...
<body style="font-family: Arial, Helvetica, sans-serif; font-size:16px">
    ...
    <h2>Shipping Address</h2>
        <address>
            ...
            {{ $mailData['order']->city }}, {{ $mailData['order']->zip }}, {{ $mailData['order']->state }},
            {{ getContryInfo($mailData['order']->country_id)->name }}<br>
    ...
    <table cellpadding="3" cellspacing="3" borde="0" width="600 ">
        <thead>
		...
		{{ !empty($mailData['order']->coupon_code) ? '(' . $mailData['order']->coupon_code . ')' : '' }}</th>
